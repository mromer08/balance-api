...
import com.mromer.balance.person.application.port.in.command.UpdatePersonMainInfo;
import com.mromer.balance.person.application.port.in.command.UpdatePersonMainInfoCommand;
...
public class UpdatePersonMainInfoImpl implements UpdatePersonMainInfo {

    private final FindPersons findPersons;
    private final SavePerson savePerson;

    @Override
    public PersonResponseDTO updatePersonMainInfo(PersonId id, UpdatePersonMainInfoCommand command) {
        Person person = findPersons.findPersonById(id).orElseThrow(
                () -> PersonNotFoundException.forId(id));

        person.updateMainInformation(
                command.cui(),
                command.firstname(),
                command.lastname(),
                command.honorific(),
